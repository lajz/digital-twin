...
class Twin:
    ...
    METADATA = {
        "assumptions": [
            "Cells are rods with fixed width 1.0 um",
            "Length grows exponentially with rate elong_rate, reduced by crowding",
            "Symmetric division at noisy target length, daughters inherit parent angle",
            "Positions updated by soft repulsion between overlapping capsules (position-based dynamics)",
            "Nematic alignment torque from contact forces",
            "Noise on positions (Langevin) to mimic segmentation error and colony expansion",
            "No death; cells stay in 2D",
            "Neighbor search via cKDTree, cutoff = 2*max_length + margin",
            "Internal timestep ~18s, ~5 steps per frame",
            "Cap cell count at 3000",
        ],
    ...
    def fit(self, obs):
        ...
        # Exponential growth rate from population count (log2)
        ...
        # Stiffness from initial NN distance and colony stability
        init_nn = nn_dist[0]
        stiffness = 5.0 + 10.0 * (2.5 - init_nn)
    ...
    def simulate(self, params, init_cells, time_s, seed):
        ...
        width = 1.0  # um
        dt = 18.0    # seconds per internal step
        ...
        while frame_idx < len(time_s):
            ...
            for _ in range(n_steps):
                # Elongation with crowding feedback (global density)
                if len(x) > 10:
                    pts = np.stack([x, y], axis=1)
                    tree = cKDTree(pts)
                    dist, _ = tree.query(pts, k=2)
                    mean_nn = np.mean(dist[:, 1])
                    # Feedback: reduce growth when NN < 1.5um
                    density_factor = 1.0 - 0.5 * np.clip((1.5 - mean_nn) / 1.0, 0, 1)
                else:
                    density_factor = 1.0
                ...
                if np.any(mask_div):
                    ...
                    for i, idx in enumerate(idx_div):
                        L = length[idx]
                        half = L / 2.0
                        # Place daughters at parent position, then push apart later
                        new_x[i] = x[idx]
                        new_y[i] = y[idx]
