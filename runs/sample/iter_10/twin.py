...
class Twin:
    ...
    FAMILY = "cell-cycle-two-phase-delay"
    ...
    PARAMS = {
        ...
        "lag":       (0.0, 1.0, "h"),     # initial lag time before growth starts
    }
    ...
    METADATA = {
        "assumptions": [
            ...
            "an initial lag period (before t=lag) has no growth; cells start dividing after lag"
        ],
        "state_vars": ["N_G1", "N_S"],
        "refs": ["Smith & Martin 1973", "Cooper 1991", "Baranyi & Roberts 1994"],
    }
    ...
    def fit(self, obs) -> dict[str, float]:
        ...
        # Model equations with lag: before lag, no growth; after lag, cell cycle proceeds.
        def model(theta, t):
            n0, k_g1, k_div, frac_s, lag = theta
            # Shift time so that growth starts at t=lag
            t_shift = np.maximum(t - lag, 0.0)
            def rhs(tau, z):
                Ng, Ns = z
            ...
            sol = solve_ivp(rhs, [0, t_shift[-1]], z0, t_eval=t_shift,
        ...
        lb = [1.0, 0.1, 0.1, 0.01, 0.0]
        ub = [1e4, 3.0, 3.0, 0.99, 1.0]
        p0 = [n0_init, k_init, k_init, frac_init, lag_init]
        
        def residual(theta):
            n0, k_g1, k_div, frac_s, lag = theta
            if k_g1 <= 0 or k_div <= 0 or frac_s <= 0 or frac_s >= 1 or lag < 0 or lag > 1:
        ...
        for restart in range(30):
            if restart > 0:
                p0_pert = [
                    ...
                    np.clip(lag_init + rng.normal(0, 0.05), lb[4], ub[4]),
                ]
            else:
                p0_pert = p0
            try:
        ...
        if best_theta is None:
            best_theta = [n0_init, k_init, k_init, frac_init, lag_init]
        
        n0, k_g1, k_div, frac_s, lag = best_theta
        ...
        lag = float(np.clip(lag, lb[4], ub[4]))
    ...
    def predict(self, params: dict, time_s: np.ndarray) -> np.ndarray:
        ...
        t_shift = np.maximum(t_h - lag, 0.0)
        def rhs(tau, z):
            Ng, Ns = z
            dNg = -k_g1*Ng + 2*k_div*Ns
            dNs =  k_g1*Ng - k_div*Ns
